...
public class MatriculaRepository : IMatriculaRepository
{
    ...
    {
        return await _context.Matricula.Include(u => u.Usuario).Include(t => t.Turma).Where(m => !m.Excluido).ToListAsync();
    }

    public async Task<Matricula> GetByIdAsync(int id)
    {
        return await _context.Matricula.Include(u => u.Usuario).Include(t => t.Turma).Where(m => !m.Excluido && m.Id == id).FirstOrDefaultAsync();
    }
